...
def graph_choices():
    ...
    # level (str): the level the user has chosen, if they have chosen one
    level = selected_level # initialize to None as 2/4 cases of graphs dont't use the level
    ...
    graph.graph_data(category, level=level, faculty_only=selected_faculty, class_count=selected_class_count)
    
    if selected_grading:
        img = Image.open('As_graph.jpg')
    else:
        img = Image.open('DsFs_graph.jpg')
